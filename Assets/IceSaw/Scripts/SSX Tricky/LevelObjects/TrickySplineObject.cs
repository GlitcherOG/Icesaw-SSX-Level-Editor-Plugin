...
public class TrickySplineObject : TrickyBaseObject
{
    ...
    public void LoadSpline(SplineJsonHandler.SplineJson spline)
    {
        ...
        {
            var Points = spline.Segments[i];

            var Point0 = ConvertLocalPoint(JsonUtil.Array2DToVector3(spline.Segments[i].Points, 0));
            var Point1 = ConvertLocalPoint(JsonUtil.Array2DToVector3(spline.Segments[i].Points, 1));
            var Point2 = ConvertLocalPoint(JsonUtil.Array2DToVector3(spline.Segments[i].Points, 2));
            var Point3 = ConvertLocalPoint(JsonUtil.Array2DToVector3(spline.Segments[i].Points, 3));

            if (i == 0)
            {
                BezierKnot bezierKnot0 = new BezierKnot(Point0, Vector3.zero, Point1);

                bezierKnot0.Position = Point0;
                bezierKnot0.TangentOut = Point1;
                bezierKnot0.Rotation = Quaternion.identity;

                splineContainer.Spline.Add(bezierKnot0);
            }
            BezierKnot bezierKnot = new BezierKnot();
            bezierKnot.Position = Point3;
            bezierKnot.TangentIn = Point2 - Point3;
            bezierKnot.Rotation = Quaternion.identity;
            //bezierKnot.TangentOut = Point3 - Point2;

            if (i != spline.Segments.Count - 1)
            {
                bezierKnot.TangentOut = ConvertLocalPoint(JsonUtil.Array2DToVector3(spline.Segments[i+1].Points, 1)) - Point3;
            }
